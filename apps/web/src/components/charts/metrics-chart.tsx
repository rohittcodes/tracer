...
export function MetricsChart({ metrics, metricType, service }: MetricsChartProps) {
  const filteredMetrics = metrics.filter(
    (m) => m.metricType === metricType && (!service || m.service === service)
  );
  ...
  const groupedData = services.reduce((acc, svc) => {
    ...
    serviceData.forEach((d) => {
      ...
      if (existing) {
        existing[svc] = d.value;
      } else {
        acc.push({ time: d.time, [svc]: d.value });
      }
    });
    return acc;
